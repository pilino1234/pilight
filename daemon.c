...
static void *broadcast(int reason, void *param) {
  ...
  switch(reason) {
    case REASON_CODE_RECEIVED: {
      ...
      uuid = data->uuid;
      repeats = data->repeat;
      len = snprintf(conf, 1024,
                     "{\"message\":%s,\"origin\":\"%s\",\"protocol\":\"%s\",\"uuid\":\"%s\",\"repeats\":%d}",
    ...
    if((protocol->hwtype == hwtype || protocol->hwtype == -1 || hwtype == -1) &&
      ...
      if(protocol->validate() == 0) {
        ...
        if(protocol->parseCode != NULL) {
          ...
          logprintf(LOG_DEBUG, "caught minimum # of repeats %d of %s", protocol->repeats,
                    protocol->id);
...
static int control_device(char *dev, char *state, struct JsonNode *values,
      ...
      while(jvalues) {
        if((opt->conftype == DEVICES_VALUE || opt->conftype == DEVICES_OPTIONAL) &&
          ...
          if(jvalues->tag == JSON_STRING) {
            ...
            json_append_member(code, jvalues->key, json_mknumber(jvalues->number_,
                               jvalues->decimals_));
          }
        }
...
static void client_webserver_parse_code(int i) {
  ...
  if(getsockname(i, (struct sockaddr *)&sockin, &len) == -1) {
    ...
    if(webserver_enable == 0 || (webserver_http_port < 0 && webserver_https_port < 0)) {
      ...
      l = snprintf(line, 255, "The pilight webserver is running at https://%s:%d/", ip,
                   webserver_https_port);
    } else if(webserver_https_port < 0) {
      l = snprintf(line, 255, "The pilight webserver is running at http://%s:%d/", ip,
                   webserver_http_port);
    } else {
      l = snprintf(line, 255,
                   "The pilight webserver is running at http://%s:%d/ or https://%s:%d/", ip,
                   webserver_https_port, ip, webserver_http_port);
    }
            ...
            if(strcmp(type, "set") == 0) {
              if(json_find_string(json, "key", &key) != 0) {
                ...
                if(value->tag == JSON_NUMBER) {
                  if(registry_update(ORIGIN_MASTER, key, json_mknumber(value->number_,
                                     value->decimals_)) == 0) {
                    if((*respons = MALLOC(strlen("{\"status\":\"success\"}") + 1)) == NULL) {
                      OUT_OF_MEMORY
                    }
  ...
  if(json_validate(data->buffer) == true) {
    ...
    if((json_find_string(json, "action", &action)) == 0) {
      if(strcmp(action, "identify") == 0) {
        ...
        if(strcmp(status, "success") == 0) {
          logprintf(LOG_DEBUG, "client \"%s\" successfully executed our latest request",
                    client->uuid);
          json_delete(json);
          return NULL;
...
void registerVersion(void) {
  ...
  registry_update(ORIGIN_MASTER, "pilight.version.current",
                  json_mkstring((char *)PILIGHT_VERSION));
}

static void pilight_stats(uv_timer_t *timer_req) {
  int watchdog = 1, stats = 1;
  double itmp = 0.0;
  if(settings_select_number(ORIGIN_MASTER, "watchdog-enable", &itmp) == 0) {
    watchdog = (int)itmp;
  }
  if(settings_select_number(ORIGIN_MASTER, "stats-enable", &itmp) == 0) {
    stats = (int)itmp;
  }
  ...
  if(stats == 1) {
    ...
    if(watchdog == 1 && (cpu > 90)) {
       ...
       */
      snprintf(data->message, 1024, "{\"values\":{\"cpu\":%f},\"origin\":\"core\",\"type\":%d}",
               cpu, PROCESS);
...
int start_pilight(int argc, char **argv) {
...
#ifdef WEBSERVER
  {
    ...
    if(file_get_contents(pemfile, &content) == 0) {
      ...
      registry_update(ORIGIN_MASTER, "webserver.ssl.certificate.location",
                      json_mkstring(pemfile));
      FREE(content);
    }
    if(pem_free == 1) {
      FREE(pemfile);
...
#endif
    double itmp = 0;
    if(settings_select_number(ORIGIN_MASTER, "webserver-enable", &itmp) == 0) {
      webserver_enable = (int)itmp;
    }
    if(settings_select_number(ORIGIN_MASTER, "webserver-http-port", &itmp) == 0) {
      webserver_http_port = (int)itmp;
    }
    if(settings_select_number(ORIGIN_MASTER, "webserver-https-port", &itmp) == 0) {
      webserver_https_port = (int)itmp;
    }
            ...
            while(tmp) {
              if(tmp->listener->hwtype == hardware->hwtype) {
                ...
                if(tmp->listener->rawlen > 0) {
                  logprintf(LOG_EMERG,
                            "%s: setting \"rawlen\" length is not allowed, use \"minrawlen\" and \"maxrawlen\" instead",
                            tmp->listener->id);
                  goto clear;
                }
              }
  ...
  }
  {
    ...
    if(settings_select_number(ORIGIN_MASTER, "port", &itmp) == 0) {
      port = (int)itmp;
    }
    if(settings_select_number(ORIGIN_MASTER, "standalone", &itmp) == 0) {
      standalone = (int)itmp;
    }
